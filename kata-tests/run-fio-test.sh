...
output_meta_info() {
  ...
  echo "RUNTIME=$RUNTIME LOOPS=$LOOPS IODEPTH=$IODEPTH SIZE=$SIZE DIRECT=$DIRECT BLOCKSIZE=$BLOCKSIZE NUMJOBS=$NUMJOBS"
  echo ""
}
...
run_test() {
  ...
  fio_cmd="fio --directory=$TESTDIR --runtime=$RUNTIME --iodepth=$IODEPTH --size="$SIZE" --direct=$DIRECT --blocksize="$BLOCKSIZE" --numjobs="$NUMJOBS" --append-terse $1"
...
Options:
  ...
  --runtime		         Time in seconds for which each job should run. Default is 60 seconds.
